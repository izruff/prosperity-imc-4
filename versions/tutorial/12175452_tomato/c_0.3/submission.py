...
class Trader(TutorialTrader):
    def trade_tomatoes(self):
        ...
        # Idea: adjust fair price according to position to maintain
        # a more neutral position.
        # If too long, we want to encourage selling: lower fair price
        # If too short, we want to encourage buying: higher fair price
        fair_price = (worst_bid + worst_ask) / 2 + CONFIG["c"] * self.position["TOMATOES"]
        lower_fair_price = math.floor(fair_price)
        upper_fair_price = math.ceil(fair_price)
        ...
        # Market make on both sides only when spread is wide enough.
