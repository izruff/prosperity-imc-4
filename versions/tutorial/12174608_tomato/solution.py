...
class Trader(TutorialTrader):
    def trade_tomatoes(self):
        ...
        # Immediately buy/sell orders strictly better than the fair price.
        if (lower_fair_price == upper_fair_price):
            self.match_buy_with_sell("TOMATOES", lower_fair_price - 1)
            self.match_sell_with_buy("TOMATOES", upper_fair_price + 1)
        else:
        ...
        # Balance the position by buying/selling at the fair price
        # In this attempt, we don't wait for a certain level of deviation,
        # because the previous strategy that didn't employ this had a
        # tendency of making a large long position.
        if (lower_fair_price == upper_fair_price):
            if self.position["TOMATOES"] < 0:
                self.send_buy_order("TOMATOES", lower_fair_price,
                                    min(-self.position["TOMATOES"],
                                        self.max_buy_orders_left("TOMATOES")),
                                    "balance")
            elif self.position["TOMATOES"] > 0:
                self.send_sell_order("TOMATOES", upper_fair_price,
                                     min(self.position["TOMATOES"],
                                         self.max_sell_orders_left("TOMATOES")),
                                     "balance")
