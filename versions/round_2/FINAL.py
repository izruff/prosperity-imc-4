...
class Trader(RoundTwoTrader):
    def trade_ash_coated_osmium(self):
        ...
        self.match_buy_with_sell(product, fair_price - CONFIG["of"], msg="for_profit")
        self.match_sell_with_buy(product, fair_price + CONFIG["of"], msg="for_profit")
        ...
        and current_pos < mm_threshold:
            self.send_buy_order(product, best_bid + 1,
                                self.max_buy_orders_left(product), msg="for_mm")
        ...
        and current_pos > -mm_threshold:
            self.send_sell_order(product, best_ask - 1,
                                 self.max_sell_orders_left(product), msg="for_mm")
    ...
    def trade_intarian_pepper_root(self):
        ...
        if self.worst_ask(product) is not None:
            self.match_buy_with_sell(product, self.worst_ask(product), msg="initial_buy")
