...
class Trader(RoundOneTrader):
    ...
    def trade_intarian_pepper_root(self):
        ...
        # 5. Market make around the fair price
        # However we assume trend is always up, so we limit sell orders
        # to only at most our long position.
        best_bid = self.best_bid(product)
        ...
        best_ask = self.best_ask(product)
        acceptable_qty = min(self.position[product],
                             self.max_sell_orders_left(product))
        if best_ask is not None and best_ask > fair_price \
        and acceptable_qty > 0:
            self.send_sell_order(product, best_ask - 1, acceptable_qty)
