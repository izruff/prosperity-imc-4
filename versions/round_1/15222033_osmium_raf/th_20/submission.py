...
class Trader(RoundOneTrader):
    def trade_ash_coated_osmium(self):
        ...
        if self.worst_ask(product) is None or self.best_bid(product) is None:
            lower_fair_price = upper_fair_price = 10000  # historical fair price
        else:
            twice_mid_price = self.worst_ask(product) + self.worst_bid(product)
            lower_fair_price = twice_mid_price // 2
            upper_fair_price = twice_mid_price - lower_fair_price
        current_pos = self.position[product]

        # 1. Execute profitable trades but turn off one side
        # if position is too skewed.
        self.match_buy_with_sell(product, lower_fair_price,
                                 CONFIG["th"] + current_pos)
        self.match_sell_with_buy(product, upper_fair_price,
                                 CONFIG["th"] - current_pos)

        # 2. Market make around the fair price.
        best_bid = self.best_bid(product)
